...
function internalUpdate( amountBetweenUpdate ){
  if( idUpdate === true ) {
    update()
    .then(()=>{
      setTimeout( internalUpdate, amountBetweenUpdate );
    });
  }
}
export function doUpdate( amountBetweenUpdate) {
  idUpdate = true;
  internalUpdate(amountBetweenUpdate);
}
