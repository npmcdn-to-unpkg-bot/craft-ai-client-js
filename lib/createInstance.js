...
function createInstance(cfg) {
  ...
  if (!_lodash2.default.has(cfg, 'name') || !_lodash2.default.isString(cfg.name)) {
  ...
  if (!_lodash2.default.has(cfg, 'version') || !_lodash2.default.isString(cfg.version)) {
  ...
  var request = function request(r) {
    ...
    return (0, _isomorphicFetch2.default)(r.url, r).catch(function (err) {
      return Promise.reject(new errors.CraftAiNetworkError({
        more: err.message
      }));
    }).then(function (res) {
      return res.json().catch(function (err) {
        return Promise.reject(new errors.CraftAiInternalError('Internal Error, the craft ai server responded an invalid json document.', {
          request: r
        }));
      }).then(function (res_content) {
        switch (res.status) {
          case 200:
            return res_content;
          case 401:
  ...
  return request({
    ...
    var instance = {
      ...
      createAgent: function createAgent(behavior) {
        ...
        if (_lodash2.default.isUndefined(behavior)) {
      ...
      updateAgentKnowledge: function updateAgentKnowledge(agentId) {
        var knowledge = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];
      ...
      updateInstanceKnowledge: function updateInstanceKnowledge() {
        var knowledge = arguments.length <= 0 || arguments[0] === undefined ? {} : arguments[0];
