...
/***/ function(module, exports, __webpack_require__) {
	...
	window.craftai = {
	  createClient: _index.createClient,
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.errors = exports.DEFAULT = exports.createClient = undefined;
	...
	exports.createClient = _client2.default;
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.CraftAiUnknownError = exports.CraftAiNetworkError = exports.CraftAiInternalError = exports.CraftAiError = exports.CraftAiCredentialsError = exports.CraftAiBadRequestError = undefined;
	...
	var CraftAiBadRequestError = createCustomError('CraftAiBadRequestError', 'Bad Request, see err.more for details');

	exports.CraftAiBadRequestError = CraftAiBadRequestError;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _onExit = __webpack_require__(11);
	...
	var debug = (0, _debug2.default)('craft-ai:client');

	function getPosixTimestamp(ts) {
	  if (_lodash2.default.isUndefined(ts)) {
	    return Math.floor(Date.now() / 1000);
	  } else if (_lodash2.default.isNumber(ts)) {
	    return Math.floor(ts);
	  } else if (_lodash2.default.isDate()) {
	...
	function createClient(cfg) {
	  cfg = _lodash2.default.defaults(_lodash2.default.clone(cfg), _defaults2.default);

	  // Initialization check
	  if (!_lodash2.default.has(cfg, 'token') || !_lodash2.default.isString(cfg.token)) {
	  ...
	  if (!_lodash2.default.has(cfg, 'url') || !_lodash2.default.isString(cfg.url)) {
	  ...
	  if (!_lodash2.default.has(cfg, 'owner') || !_lodash2.default.isString(cfg.owner)) {
	  ...
	  var agentsToDestroyOnExit = [];
	  (0, _onExit2.default)(function () {
	    if (agentsToDestroyOnExit.length > 0) {
	      debug('Destroying agents ' + _lodash2.default.map(agentsToDestroyOnExit, function (agent) {
	        return '\'' + agent + '\'';
	      }).join(', ') + ' before exiting...');
	      _lodash2.default.forEach(agentsToDestroyOnExit, function (agentId) {
	        (0, _request2.default)({
	          method: 'DELETE',
	          path: '/agents/' + agentId,
	          asynchronous: false
	  ...
	  var agentsOperations = {};

	  // The promise that actually flush the context operations for one agent.
	  var flushAgentContextOperationsPromise = function flushAgentContextOperationsPromise(agentId) {
	    // Extract the operations to flush
	    var operationsToFlush = agentsOperations[agentId] || [];
	    ...
	    if (operationsToFlush.length === 0) {
	      ...
	      return (0, _lodash2.default)(operationsToFlush).chunk(cfg.operationsChunksSize).reduce(function (p, chunk) {
	  ...
	  var currentFlushOperation = new Promise(function (resolve) {
	    return resolve();
	  });

	  // To execute when an immediate flush is needed.
	  var flushAgentContextOperations = function flushAgentContextOperations(agentId) {
	    currentFlushOperation = currentFlushOperation.then(function () {
	      return flushAgentContextOperationsPromise(agentId);
	    });
	    return currentFlushOperation.catch(function (err) {
	      ...
	      currentFlushOperation = new Promise(function (resolve) {
	        return resolve();
	      });
	  ...
	  var throttledFlushAgentContextOperations = _lodash2.default.throttle(function (agentId) {
	    currentFlushOperation = currentFlushOperation.then(function () {
	      return flushAgentContextOperationsPromise(agentId);
	    });
	  ...
	  var instance = _lodash2.default.defaults(_lodash2.default.clone(cfg), _defaults2.default, {
	    cfg: cfg,
	    createAgent: function createAgent(model) {
	      var id = arguments.length <= 1 || arguments[1] === undefined ? undefined : arguments[1];
	      var destroyOnExit = arguments.length <= 2 || arguments[2] === undefined ? false : arguments[2];

	      if (_lodash2.default.isUndefined(model) || !_lodash2.default.isObject(model)) {
	    ...
	    destroyAgent: function destroyAgent(agentId) {
	      if (_lodash2.default.isUndefined(agentId)) {
	    ...
	    getAgentContext: function getAgentContext(agentId) {
	      var _this2 = this;

	      var timestamp = arguments.length <= 1 || arguments[1] === undefined ? undefined : arguments[1];
	      ...
	      var posixTimestamp = getPosixTimestamp(timestamp);
	      if (_lodash2.default.isUndefined(posixTimestamp)) {
	    ...
	    addAgentContextOperations: function addAgentContextOperations(agentId, operations) {
	      ...
	      if (_lodash2.default.isUndefined(operations) || !_lodash2.default.isArray(operations)) {
	    ...
	    getAgentContextOperations: function getAgentContextOperations(agentId) {
	      var _this3 = this;
	    ...
	    getAgentInspectorUrl: function getAgentInspectorUrl(agentId, timestamp) {
	      if (_lodash2.default.isUndefined(agentId)) {
	      ...
	      if (_lodash2.default.isUndefined(timestamp)) {
	    ...
	    getDecisionTree: function getDecisionTree(agentId, timestamp) {
	      var _this4 = this;
	    ...
	    computeAgentDecision: function computeAgentDecision(agentId, timestamp, context) {
	      var _this5 = this;
	      ...
	      if (_lodash2.default.isUndefined(context) || !_lodash2.default.isObject(context)) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var DEFAULTS = {
	  token: (undefined),
	  url: (undefined) || 'https://beta.craft.ai',
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _syncRequest = __webpack_require__(16);
	...
	function parseBody(req, resBody) {
	  var resBodyUtf8 = void 0;
	  try {
	    resBodyUtf8 = resBody.toString('utf-8');
	  } catch (err) {
	    debug('Invalid response format from ' + req.method + ' ' + req.path + ': ' + resBody, err);
	    throw new errors.CraftAiInternalError('Internal Error, the craft ai server responded in an invalid format.', {
	      request: req
	    });
	  }
	  var resBodyJson = void 0;
	  try {
	    ...
	    throw new errors.CraftAiInternalError('Internal Error, the craft ai server responded an invalid json document.', {
	      request: req
	    });
	  }
	  return resBodyJson;
	...
	function request(req, cfg) {
	  req = _lodash2.default.defaults(req || {}, {
	    method: 'GET',
	    path: '',
	    body: undefined,
	    asynchronous: true,
	  ...
	  if (_lodash2.default.size(req.query) > 0) {
	    req.url = req.url + '?' + _lodash2.default.map(_lodash2.default.keys(req.query), function (key) {
	  ...
	  if (req.asynchronous) {
	    return (0, _isomorphicFetch2.default)(req.url, req).catch(function (err) {
	      ...
	      return res.text().catch(function (err) {
	        ...
	        return Promise.reject(new errors.CraftAiInternalError('Internal Error, the craft ai server responded an invalid response, see err.more for details.', {
	          request: req,
	          more: err.message
	        }));
	      }).then(function (resBody) {
	        return parseResponse(req, res.status, resBody);
	      });
	    });
	  } else {
	    var res = (0, _syncRequest2.default)(req.method, req.url, req);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var handleQs = __webpack_require__(18);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var stringify = __webpack_require__(19).stringify;
